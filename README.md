...
- **25+ technical indicators** — MAs, Bollinger, Keltner, Donchian, Ichimoku, Parabolic SAR, RSI, MACD, Stoch RSI, CMF, Momentum, and more
...
- **Screenshot export** — save any chart pane as a PNG with all drawings composited
...
**Telegram alerts:** Fill in `TELEGRAM_BOT_TOKEN` and `TELEGRAM_CHAT_ID` in `.env`. Alerts fire simultaneously to both browser and Telegram.
...
- **Pane popout** (the button in the pane toolbar) — pops that chart into a new window on your second screen
- **Full terminal** (the button in the main topbar) — opens a complete second Joshua Terminal on the second screen
